...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_COMMAND(ID_VIEW_PICTUREBAR, OnViewPicturebar)
	ON_MESSAGE(WM_NEWURL, OnNewURL)
	ON_MESSAGE(WM_UPOUTPUTtoSELANG, OnOutputMessage) 
	ON_MESSAGE(WM_WRURL, OnWRURL) 
	//}}AFX_MSG_MAP
	...
	ON_COMMAND(ID_PIC_BAR, &CMainFrame::OnPicBar)
	ON_UPDATE_COMMAND_UI(ID_PIC_BAR, &CMainFrame::OnUpdatePicBar)
	ON_WM_CLOSE()
	ON_COMMAND(ID_INDICATOR_LABEL, &CMainFrame::OnIndicatorLabel)
	ON_COMMAND(ID_LINK_8564_COM, &CMainFrame::OnLink8564Com)
	ON_COMMAND_RANGE(ID_BEGINID,ID_ENDID,&CMainFrame::ONOpenWebsite)
END_MESSAGE_MAP()
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	///////////////////////////////////
	if (!m_mOutputBar.Create(_T("下载链接显示"), this, CRect(0, 0, 100, 100), TRUE, ID_PIC_BAR, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_BOTTOM | CBRS_FLOAT_MULTI))
	{
		TRACE0("Failed to create output bar\n");
		return FALSE;      // fail to create
	}
	m_mOutputBar.EnableDocking(CBRS_ALIGN_ANY);
	DockPane(&m_mOutputBar);
	//-------------------
	// Create status bar:
	//-------------------
	...
	m_wndStatusBar.SetPaneTextColor(nStatusText,RGB(255,0,0),1);
	...
	case ID_GO_FORWARD:
		{
			...
			CPICDoloView* pView = ((CPICDoloView*)GetActiveView());
			ASSERT_VALID (pView);

			CPICDoloDoc* pDoc = pView->GetDocument();
			ASSERT_VALID(pDoc);
...
{
	SetFocus ();
	((CPICDoloView*)GetActiveView())->Navigate2(theApp.m_Favorites.GetURLofID (nID), 0, NULL);
}
...
void CMainFrame::OnNewAddress()
{
	...
	((CPICDoloView*)GetActiveView())->Navigate2(str, 0, NULL);
}
...
void CMainFrame::OnViewPicturebar() 
{
	ShowPane (&m_wndExplorerBar, (m_wndExplorerBar.GetStyle () & WS_VISIBLE) == 0, FALSE, TRUE);
}
...
LRESULT CMainFrame::OnNewURL(WPARAM wParam, LPARAM lParam)
{
	...
	//CMainFrame* pChildFrame = (CMainFrame*)GetActiveFrame();
	CMainFrame* pChildFrame = (CMainFrame*)AfxGetMainWnd(); 
	*ppDispatch=((CPICDoloView*)pChildFrame->GetActiveView())->GetApplication();
	return 0;
}

void CMainFrame::updataTaskString(CString str)
{
	outputString(str);
	//m_wndExplorerBar.AddTaskString(str);
	//m_wndExplorerBar.Update();
}


void CMainFrame::OnPicBar()
{
	// TODO: 在此添加命令处理程序代码
	ShowPane (&m_mOutputBar, (m_mOutputBar.GetStyle () & WS_VISIBLE) == 0, FALSE, TRUE);
}


void CMainFrame::OnUpdatePicBar(CCmdUI *pCmdUI)
{
	// TODO: 在此添加命令更新用户界面处理程序代码
	pCmdUI->SetCheck (m_mOutputBar.IsWindowVisible ());
}


// 显示output内容
bool CMainFrame::outputString(CString& str)
{
	m_mOutputBar.OutputString(str);	
	return false;
}
...
void CMainFrame::OnClose()
{
	...
	CPICDoloDoc *pdoc= (CPICDoloDoc*)pFrame->GetActiveView()->GetDocument();	
	pdoc->SaveALLMyFile();
	b_closeP = true;
	CFrameWnd::OnClose();
}
int dolo_num=0;
LRESULT CMainFrame::OnOutputMessage(WPARAM wParam, LPARAM lParam) 
{ 
	// TODO: 处理用户自定义消息
	CString str;
	...
	CString str1=str.Right(str.GetLength()-i-1);
	pdoc->hasDownPic[str]=true;
	//pdoc->mhashmap_url[str]=str1;
	CString cs_i;
	dolo_num++;
	cs_i.Format(_T("%4d:"),dolo_num);
	cs_i+=str1;
	m_mOutputBar.OutputString(cs_i);
	return 0; 
}

LRESULT CMainFrame::OnWRURL(WPARAM wParam, LPARAM lParam) 
{
	CMainFrame *pFrame = (CMainFrame *)AfxGetApp()->m_pMainWnd;
	CPICDoloDoc *pdoc= (CPICDoloDoc*)pFrame->GetActiveView()->GetDocument();
	CString str;
	str.Format(_T("%s"),(char *)wParam);
	int i = str.Find(_T('/'),8);
	CString str1=str.Left(i);
	pdoc->mhashmap_url[str]=str1;
	return 0;
}


void CMainFrame::OnIndicatorLabel()
{
	// TODO: 在此添加命令处理程序代码
}
...
{
	((CPICDoloView*)GetActiveView())->Navigate2 (_T("http://www.moko.cc"), 0, NULL);
}
//********************************************************************************************
void CMainFrame::OnLinkMicrosoft()
{
	((CPICDoloView*)GetActiveView())->Navigate2 (_T("http://huabao.taobao.com"), 0, NULL);
}
//********************************************************************************************
void CMainFrame::OnLinkVisualCPP()
{
	((CPICDoloView*)GetActiveView())->Navigate2 (_T("http://mm.taobao.com"), 0, NULL);
}
//********************************************************************************************
void CMainFrame::OnLink8564Com()
{
	((CPICDoloView*)GetActiveView())->Navigate2 (_T("http://www.8264.com/pp/"), 0, NULL);
}
//extern struct ST_URL;
extern vector<struct ST_URL> myUrl;

void CMainFrame::ONOpenWebsite(UINT nID)
{
	int n;
	n=nID;
	for(unsigned int i=0;i<myUrl.size();i++)
	{
		if((unsigned int )myUrl[i].id== nID)
		{
			((CPICDoloView*)GetActiveView())->Navigate2 (myUrl[i].url, 0, NULL);
			break;
		}
	}
}
